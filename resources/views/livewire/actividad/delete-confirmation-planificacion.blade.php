...
<x-dialog-modal wire:model.live="showDeletePlanificacionModal" maxWidth="md">
    <x-slot name="title">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <h4 class="text-lg font-medium text-zinc-900 dark:text-zinc-100">
                                {{ $planificacionToDelete->indicador->nombre ?? 'Planificación' }}
                            </h4>
                            <p class="text-sm text-zinc-500 dark:text-zinc-400">
                                Cantidad: {{ $planificacionToDelete->cantidad }} | Trimestre
                                {{ $planificacionToDelete->mes->trimestre->trimestre ?? '' }}
                            </p>
                        </div>
                    </div>
                </div>
    ...
    <x-slot name="footer">
        <x-secondary-button wire:click="closeDeletePlanificacionModal">
            Cancelar
        </x-secondary-button>

        <x-danger-button class="ml-3" wire:click="confirmDeletePlanificacion">
            Eliminar
        </x-danger-button>
    </x-slot>
</x-dialog-modal>