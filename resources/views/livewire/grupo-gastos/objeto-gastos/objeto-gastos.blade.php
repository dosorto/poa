<div>
    <div class=" mx-auto rounded-lg mt-8 sm:mt-6 lg:mt-4 mb-6">
        <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow sm:rounded-lg p-4 sm:p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                <h2 class="text-xl font-semibold text-zinc-800 dark:text-zinc-200">{{ __('Administración de Objetos de Gasto') }}</h2>

                <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                    <div class="relative w-full sm:w-auto">
                        <x-input wire:model.live="search" type="text" placeholder="Buscar objetos de gasto..."
                    ...
                    @can('configuracion.objetogastos.crear')
            ...
            <x-table
                ...
                empty-message="{{ __('No se encontraron objetos de gasto') }}"
                class="mt-6"
            >
                <x-slot name="desktop">
                    @forelse ($objetosGasto as $objeto)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-zinc-900 dark:text-zinc-300">
                                <span class="bg-emerald-100 text-emerald-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-emerald-900 dark:text-emerald-200">
                                    {{ $objeto->identificador }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-zinc-900 dark:text-zinc-300">
                                <div class="font-medium">{{ $objeto->nombre }}</div>
                                <div class="text-xs text-zinc-500 dark:text-zinc-400 line-clamp-2">{{ $objeto->descripcion }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-zinc-900 dark:text-zinc-300">
                                {{ $objeto->grupoGasto?->identificador }} - {{ $objeto->grupoGasto?->nombre ?? 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    @can('configuracion.objetogastos.editar')
                                    <button wire:click="edit({{ $objeto->id }})"
                                    ...
                                    @endcan
                                    @can('configuracion.objetogastos.eliminar')
                                    <button wire:click="confirmDelete({{ $objeto->id }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-4 text-center text-zinc-500 dark:text-zinc-400">
                                {{ __('No se encontraron objetos de gasto') }}
                            </td>
                        </tr>
                    @endforelse
                </x-slot>
                ...
                <x-slot name="mobile">
                    @forelse ($objetosGasto as $objeto)
                        <div class="bg-white dark:bg-zinc-800 p-4 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 mb-4">
                            ...
                            <h3 class="font-semibold text-zinc-900 dark:text-zinc-200 text-lg mb-1">{{ $objeto->nombre }}</h3>
                            <div class="mt-2 space-y-2">
                                <span class="bg-emerald-100 text-emerald-800 text-sm font-medium px-2.5 py-0.5 rounded dark:bg-emerald-900 dark:text-emerald-200">
                                    {{ $objeto->identificador }}
                                </span>
                                <p class="text-sm text-zinc-600 dark:text-zinc-400">{{ $objeto->descripcion }}</p>
    ...
    @include('livewire.grupo-gastos.objeto-gastos.create')

    @include('livewire.grupo-gastos.objeto-gastos.delete-confirmation')
